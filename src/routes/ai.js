...
router.post('/parse-invoice', body('description').isString().notEmpty(), async (req, res) => {
    try {
...
// Generate contract
router.post('/generate-contract', body('invoiceData').isObject().notEmpty(), body('templateType').optional().isString(), async (req, res) => {
    try {
...
// Risk analysis
router.post('/analyze-risk', body('invoiceData').isObject().notEmpty(), async (req, res) => {
    try {
